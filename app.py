...
st.markdown('<p class="main-header">🛍️ Shopizz Analytics Dashboard</p>', unsafe_allow_html=True)
...
with tab1:
    st.markdown('<p class="sub-header">🔗 Association Rule Mining</p>', unsafe_allow_html=True)
    ...
    if st.button("🔍 Run Association Rule Mining", key="arm_run"):
...
with tab2:
    st.markdown('<p class="sub-header">🎯 Multi-Class Classification</p>', unsafe_allow_html=True)
    ...
    if st.button("🚀 Run Classification", key="class_run"):
...
with tab3:
    st.markdown('<p class="sub-header">👥 Customer Segmentation</p>', unsafe_allow_html=True)
    ...
    if st.button("🎯 Run Clustering", key="cluster_run"):
        with st.spinner("Performing customer segmentation..."):
            try:
                ...
                # PCA Visualization
                st.subheader("🗺️ 2D PCA Cluster Visualization")
                        ...
                        with st.expander(f"{persona['emoji']} Cluster {cluster_id}: {persona['name']}"):
                            ...
                            with col1:
                                st.metric("Avg Monthly Spending", 
                                         f"₹{cluster_data['Monthly_Spending'].mean():.0f}")
                            with col2:
                                st.metric("Avg Fashion Spending", 
                                         f"₹{cluster_data['Fashion_Spending'].mean():.0f}")
                            with col3:
                                st.metric("Avg HomeDecor Spending", 
                                         f"₹{cluster_data['HomeDecor_Spending'].mean():.0f}")
...
with tab4:
    ...
    if st.button("📊 Run Regression", key="reg_run"):
...
with tab5:
    st.markdown('<p class="sub-header">💰 Dynamic Pricing Engine</p>', unsafe_allow_html=True)
    st.markdown("Real-time price prediction based on customer characteristics")
    
    if 'best_regression_model' not in st.session_state:
        st.warning("⚠️ Please run Regression Analysis first to enable dynamic pricing!")
        st.info("Go to the **Regression** tab and train a model.")
    else:
        st.success("✅ Pricing model loaded successfully!")
        ...
        with col3:
            monthly_spending = st.number_input("Monthly Spending (₹)", 
                                              min_value=0, max_value=100000, value=5000)
            fashion_spending = st.number_input("Fashion Spending (₹)", 
                                              min_value=0, max_value=50000, value=2000)
            homedecor_spending = st.number_input("Home Decor Spending (₹)", 
                                                min_value=0, max_value=50000, value=1500)
        
        st.markdown("### ⭐ Feature Preferences (1-5)")
        ...
        if st.button("💡 Predict Optimal Price", key="price_predict"):
            with st.spinner("Calculating optimal price..."):
                try:
                    ...
                    if monthly_spending > 20000:
                        ...
                        st.info(f"🌟 **Premium Customer**: Consider premium pricing at ₹{premium_price:.0f} (+15%)")
                    elif monthly_spending < 5000:
                        budget_price = base_price * 0.85
                        st.info(f"💰 **Budget-Conscious**: Offer competitive pricing at ₹{budget_price:.0f} (-15%)")
                    else:
                        st.info(f"📊 **Standard Pricing**: Recommend base price at ₹{base_price:.0f}")
                    
                    # Volume discount
                    st.markdown("#### 📦 Volume-Based Pricing")
                    vol_data = {
                        'Quantity': ['1 item', '2-3 items', '4-5 items', '6+ items'],
                        'Unit Price': [
                            f"₹{base_price:.0f}",
                            f"₹{base_price * 0.95:.0f} (-5%)",
                            f"₹{base_price * 0.90:.0f} (-10%)",
                            f"₹{base_price * 0.85:.0f} (-15%)"
                        ]
                    }
